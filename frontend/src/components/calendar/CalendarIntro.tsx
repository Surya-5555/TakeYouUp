...
import { useGLTF, useAnimations, Environment, PerspectiveCamera, ContactShadows, Html } from "@react-three/drei";
...
function Robot({ onAnimationComplete, onReveal }: { onAnimationComplete: () => void, onReveal: () => void }) {
  ...
  useEffect(() => {
    ...
    if (phase === "entering") {
      ...
      // 2. Stop walking, mount it to wall, give Thumbs Up
  ...
  useFrame((state, delta) => {
    if (!group.current || !calendarRef.current) return;
    
    if (phase === "entering") {
      group.current.position.x += 2 * delta;
      group.current.rotation.y = Math.PI / 2 + 0.3;

      calendarRef.current.position.x = group.current.position.x + 0.8;
      calendarRef.current.position.y = group.current.position.y + 2.5; 
      calendarRef.current.position.z = 0.8;
      calendarRef.current.rotation.y = Math.PI / 6; 
      calendarRef.current.scale.setScalar(1);
      ...
      // Thrust it out!
      calendarRef.current.rotation.y = THREE.MathUtils.lerp(calendarRef.current.rotation.y, 0, 0.1);
      calendarRef.current.position.x = THREE.MathUtils.lerp(calendarRef.current.position.x, 0, 0.1);
      calendarRef.current.position.z = THREE.MathUtils.lerp(calendarRef.current.position.z, 2.5, 0.05);
      ...
      // Hide 3D proxy immediately since real DOM calendar has been fired
      calendarRef.current.visible = false;
    }
  });
  ...
  return (
    <>
      ...
      {/* 100% Accurate 3D Component Node */}
      <group ref={calendarRef}>
        <Html transform scale={0.04} center>
          <div className="w-[1000px] pointer-events-none drop-shadow-[0_40px_80px_rgba(0,0,0,0.8)] bg-transparent">
             <TrendyCalendar />
          </div>
        </Html>
      </group>
  ...
  return (
    <div className="w-full h-full bg-transparent overflow-hidden relative select-none pointer-events-none">
      ...
      <div className="absolute bottom-12 left-0 w-full flex justify-center z-10">
        <p className="text-white/40 tracking-[0.5em] text-[10px] font-black uppercase animate-pulse">
          Hanging Calendar...
        </p>
      </div>
    </div>
